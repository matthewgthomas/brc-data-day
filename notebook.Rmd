...
First, we install and load the [`tidyverse` packages](https://www.tidyverse.org/), which will help us do a lot of the data wrangling and analysis:
...
Use tidyverse's `read_csv()` function to import data on films, actors and genres:

```{r load_data}
films = read_csv("data/Movie_Movies.csv")
actors = read_csv("data/Movie_Actors.csv")
genres = read_csv("data/Movie_Genres.csv")
```

We now have three dataframes loaded: `films`, `actors`, and `genres`. Let's take a peek at some of the data:
...
The relationship between these two variables is [statistically significant](https://towardsdatascience.com/statistical-significance-hypothesis-testing-the-normal-curve-and-p-values-93274fa32687) because the p-value is way below the standard threshold of 0.05 (`p-value < 2.2e-16`). However, The size of the correlation isn't very large (-0.038, with a [95% confidence interval](https://www.dummies.com/education/science/biology/confidence-interval-basics/) of [-0.046, -0.029]), so there's nothing very interesting going on here.
...
films %>% 
  ...
  filter(Duration == max(ratings_duration$Duration)) %>%  # filter the film with the max. duration of all films in the dataframe
...
Get each separate word from each film title (known as 'tokenisation') and count the number of times each word appears:
...
Get rid of common words like 'the' and 'a':
